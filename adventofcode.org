...
by prime numbers $p_{i}$, which is not affected by treating everything in $\mathbb{Z}/\prod_{i} p_{i}\mathbb{Z}$.

#+begin_src crystal :results output :exports both :tangle yes
require "benchmark"
...
entries.map { |monkey|
  monkey.items.each { |level|
    ...
    if level.divisible_by?(monkey.divider)
      entries[monkey.iftrue].items = entries[monkey.iftrue].items << level
    else
      entries[monkey.iffalse].items = entries[monkey.iffalse].items << level
    end
...
#+RESULTS:
:   0.132146   0.000135   0.132281 (  0.132286)
: 29703395016
