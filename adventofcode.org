...
#+begin_src cpp :results output :main no :includes <iostream> <fstream> <string> <vector> <unordered_set> :options -Wall :tangle yes
...
const std::vector<std::vector<int>> update_tail(std::vector<std::vector<int>> tails, const int dx, const int dy, std::unordered_set<std::string>& vis) {
     ...
     for (int i=0; i<abs(dy); i++){
            ...
            return tails;
             ...
             void debug_print(std::vector<std::vector<int>> vect) {
                 for(auto i : vect) {
                     std::cout << i[0] << " " << i[1] << "\n";
                 }
                 std::cout << "\n";
